...
-(void)setUpCustomerWithCardToken:(NSString*)tok email:(NSString*)email{
    NSString *urlForTag = [NSString stringWithFormat:@"https://xpand.today/stripe/subscribecustomer.php?xpand_id=%@&token=%@", userProfile.userId, tok];
//    NSString *urlForTag = [NSString stringWithFormat:@"http://192.168.0.7/individual-projects/xpand/xpand-web/site/stripe/subscribecustomer.php?xpand_id=%@&token=%@", userProfile.userId, tok];
    ...
    [NSURLConnection sendAsynchronousRequest:req queue:[[NSOperationQueue alloc] init] completionHandler:^(NSURLResponse *response, NSData *data, NSError *error) {
        if (error) {
            ...
            if (jsonError) NSLog(@"%@", jsonError);
            
//            NSLog(@"!!!! %@", jsonDictionary);
            if ([[[jsonDictionary objectForKey:@"meta"] objectForKey:@"code"] integerValue] == 200) {
                NSLog(@"Customer creation suc");
                [self.delegate customerCreated];
            }else{
                NSLog(@"Customer creation failed");
            }
        }
    }];
}

-(void)changeCustomerCardDeatils:(NSString*)tok{
    NSString *urlForTag = [NSString stringWithFormat:@"https://xpand.today/stripe/changecarddetails.php?xpand_id=%@&token=%@", userProfile.userId, tok];
    NSMutableURLRequest *req = [[NSMutableURLRequest alloc]initWithURL:[NSURL URLWithString:urlForTag]];
    [NSURLConnection sendAsynchronousRequest:req queue:[[NSOperationQueue alloc] init] completionHandler:^(NSURLResponse *response, NSData *data, NSError *error) {
        if (error) {
            NSLog(@"Error 569: %@", error);
        } else {
            NSDictionary *jsonDictionary=[NSJSONSerialization JSONObjectWithData:data options:0 error:nil];
...
-(void)unsubscribeCustomer{
    NSString *urlForTag = [NSString stringWithFormat:@"https://xpand.today/stripe/unsubscribe.php?xpand_id=%@", userProfile.userId];
    NSMutableURLRequest *req = [[NSMutableURLRequest alloc]initWithURL:[NSURL URLWithString:urlForTag]];
    [NSURLConnection sendAsynchronousRequest:req queue:[[NSOperationQueue alloc] init] completionHandler:^(NSURLResponse *response, NSData *data, NSError *error) {
        if (error) {
            NSLog(@"Error 1447: %@", error);
        } else {
            NSDictionary *jsonDictionary=[NSJSONSerialization JSONObjectWithData:data options:0 error:nil];
...
-(void)non200ReceivedWithString:(NSString*)str{
    ...
    if ([self.delegate respondsToSelector:@selector(JSONReceived:)]) {
